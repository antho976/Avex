...
class StandingEngineTest {
    ...
    @Test fun `emits one metric per dimension`() {
        val m = StandingEngine.standings(StandingSnapshot(3.0, 8, 25_000.0))
    ...
    @Test fun `more is better - top percent decreases as the metric rises`() {
        val low = StandingEngine.standings(StandingSnapshot(1.0, 1, 5_000.0))
        val high = StandingEngine.standings(StandingSnapshot(6.0, 40, 110_000.0))
    ...
    @Test fun `percentile is interpolated between anchors`() {
        ...
        val m = StandingEngine.standings(StandingSnapshot(3.5, 0, 0.0)).first { it.key == "consistency" }
        assertTrue(m.topPercent in 20..25)
    }

    @Test fun `percentile stays within the 2 to 99 band`() {
        StandingEngine.standings(StandingSnapshot(0.0, 0, 0.0)).forEach {
            assertTrue(it.topPercent in 2..99)
        }
        StandingEngine.standings(StandingSnapshot(99.0, 999, 9_999_999.0)).forEach {
            assertTrue(it.topPercent in 2..99)
        }
    }
    ...
    @Test fun `deterministic`() {
        ...
        assertEquals(StandingEngine.standings(s), StandingEngine.standings(s))
    }
}
